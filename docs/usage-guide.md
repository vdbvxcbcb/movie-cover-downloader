...
详情服务优先读取豆瓣结构化 API，并用经过校验的详情页补充字段；当结构化 API 无权限但登录后的详情页可用时，会直接从详情页解析完整字段。详情弹窗会按豆瓣实际返回内容展示：

- 影片标题、原始标题和年份。
- 导演、编剧、主演、类型、制片国家/地区、语言、上映日期、片长、季数、集数、又名和 IMDb 编号。
- 豆瓣评分数字和评分人数；没有有效评分时只显示 `暂无评分`。
- 完整剧情简介和 `复制简介` 按钮；页面同时包含收起内容和展开内容时，优先使用展开后的完整简介。
...
- 上传或拖拽 1-9 张图片；一次拖入多张图片时从第一格开始按当前布局顺序自动填充，单张图片仍放入目标格。
- 选择拼版布局。
- 拖拽换格。
- 前景图缩放范围为 100%-300%；当前选中图片只要在某个方向存在超出格子的内容，就可以拖动查看其他区域，格子固定且不会露出空白。
- 每个已填充格子右上角都有左转和右转按钮，每次旋转 90 度；旋转只作用于当前格子的前景图，且始终裁剪在格子内。
- 工具栏的画布预览缩放范围为 30%-100%，`还原` 恢复 100%；导出图片的像素尺寸会随该比例变化。
- 上传背景图后可在 `当前选择` 中切换 `前景图 / 背景图`。背景图保持原比例完整显示，可缩小到 30%，选中后可在画布边界内拖动，任何部分都不会超出画布。
- 开启背景重叠会自动选中背景图；取消重叠或移除背景图会切回前景图。移除背景图时，透明度和大小都重置为 100%。
- 调整当前选中图片透明度。
- 添加文字、方框、圆圈、箭头标注；文字默认 30 号黄色，支持普通 Enter 或 Shift+Enter 换行以及左/中/右对齐。
- 拖动文字框接近画布水平或垂直中心时会吸附，并显示对应参考线。
- 点击撤销图标或按 `Ctrl+Z` 可撤销标注操作；普通输入框中的 `Ctrl+Z` 仍由输入框自身处理。
- `清除标注` 只清空标注；`清除全部` 会清空全部前景图和标注，不移除背景图。
- 导出 JPG 或 PNG。
